  ...
  req.getValidationResult().then(result => {
    ...
    if (success[0] > 1) {
      ...
      return knex('webguiSetting').select().where({
        key: 'account'
      }).then(success => JSON.parse(success[0].value)).then(success => {
          ...
          return knex('webguiSetting').select().where({
            ...
            if (port.random) {
              ...
              const checkIfPortExists = port => {
                let myPort = port;
                return knex('account_plugin').select().where({ port }).then(success => {
              ...
              return knex('account_plugin').select().whereBetween('port', [port.start, port.end]).orderBy('port', 'DESC').limit(1).then(success => {
                if (success.length) {
...
exports.checkResetPasswordToken = (req, res) => {
  ...
  knex('user').select().where({
    resetPasswordId: token
  }).whereBetween('resetPasswordTime', [Date.now() - 600 * 1000, Date.now()]).then(users => {
    if (!users.length) {
      return Promise.reject('user not exists');
    }
    return users[0];
